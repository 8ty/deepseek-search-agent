...
### 说明：
1. **环境变量**：请确保在运行之前设置 `JINA_API_KEY` 和 `OPENROUTER_API_KEY`。
2. **异步编程**：使用 `asyncio` 来处理异步请求。
3. **清除输出**：在 Linux 终端中使用 `print("\033c", end="")` 来清除输出。
4. **提示模板**：您需要在 `prompt_template` 中添加实际的提示模板内容。
5. **运行**：将此代码保存为 `.py` 文件并在 Linux 终端中运行。
